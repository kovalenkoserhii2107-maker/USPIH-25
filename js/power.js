    ...
    document.getElementById('powerToggleInput')?.addEventListener('change', async function () {
        ...
        try {
            ...
            try {
                ...
                // Перемикання лишається чинним — статус уже записано.
                // Але мовчати не можна: без запису в журналі подія
                // випадала зі статистики, і правління про це не знало.
                console.error('Журнал світла:', e.code, e);
                toast('Статус змінено, але подія не потрапила в журнал', 'error');
            }

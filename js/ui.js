...
/**
 * Посилання на файл застосунку — або порожній рядок, якщо воно веде
 * будь-куди, крім сховища цього проєкту Firebase.
 *
 * Вкладення зі звернень, чату й коментарів пишуть мешканці, і правила
 * бази до недавнього часу не перевіряли в них адресу. Через DevTools
 * можна було покласти будь-яке посилання — наприклад, на фішинговий
 * сайт під виглядом «Акт.docx», — і правління відкрило б його з
 * інтерфейсу застосунку. Тому перевіряємо не лише домен: власний
 * проєкт Firebase заведе будь-хто, і його файли теж лежатимуть на
 * firebasestorage.googleapis.com. Дозволено лише сховище «uspih-25».
 *
 * Адресу спершу розбирає URL: так «../» у шляху не виведе за межі
 * нашого бакета, а регулярний вираз перевіряє вже нормалізовану.
 */
const OWN_FILES = /^https:\/\/firebasestorage\.googleapis\.com\/v0\/b\/uspih-25\.(firebasestorage\.app|appspot\.com)\/o\//;

export function safeFileUrl(url) {
    try {
        const href = new URL(String(url || '')).href;
        return OWN_FILES.test(href) ? href : '';
    } catch {
        return '';
    }
}
